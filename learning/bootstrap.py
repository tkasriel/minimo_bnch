...
async def teacher_loop(cfg: DictConfig):
    ...
    with open('log.jsonl', 'w') as log:
        for i in range(start_iteration, cfg.iterations):
            ...
            if dsprover.distributed_state.is_main_process:
            ...
            #         instruction_queue.put((CONJECTURE, get_seed_statement())) # You can put the seed statement here
                ...
                conjecture_json = {"conjectures": conjectures, "seed_used": seed_used, "comp_to_raw_dict": comp_to_raw_dict}
                with open(f"generated_conjectures_{i}.json", "w") as f:
                    json.dump(conjecture_json, f)

            # 2- Try to prove each of the conjectures
            dsprover.distributed_state.wait_for_everyone()
            with open(f"generated_conjectures_{i}.json", "r") as f:
                conjecture_json = json.load(f)
                conjectures = conjecture_json["conjectures"]
                seed_used = conjecture_json["seed_used"]
                comp_to_raw_dict = conjecture_json["comp_to_raw_dict"]
            ...
            batches = []
            for j in range(0, len(conjectures), cfg.get("batch_size")):
                batches.append((
                                conjectures[j:min(j+cfg.get("batch_size"), len(lean_conjectures))],
                                lean_conjectures[j:min(j+cfg.get("batch_size"), len(lean_conjectures))]
                ))
            if dsprover.distributed_state.is_main_process:
                print(batches)
            # if dsprover.distributed_state.is_main_process:
            #     bar = tqdm(total=len(lean_conjectures))
            with dsprover.distributed_state.split_between_processes(batches) as batches_distrib:
                for batch in batches_distrib:
                    batch_peano = batch[0]
                    batch_lean = batch[1]
                    results = dsprover.prove(batch_lean)
                    for k, result in enumerate(results):
                        proof = result[0]
                        logprob = result[1]
                        print(batch_peano[k])
                        if proof:
                            student_results.append({"problem": batch_peano[k], "translated": batch_lean[k], "proof": result, "logprob": logprob})
                        else:
                            student_results.append({"problem": batch_peano[k], "translated": batch_lean[k], "proof": None, "logprob": 0.0})
                    
            #     bar.update(len(results))
            # bar.close()
            for rank in range(dsprover.distributed_state.num_processes):
                if dsprover.distributed_state.process_index == rank:
                    out = []
                    if os.path.exists(f"proof_results_{i}.json"):
                        with open(f"proof_results_{i}.json", "r") as f:
                            out = json.load(f)
                    out.extend(student_results)
                    with open(f"proof_results_{i}.json", "w") as f:
                        json.dump(out, f)
                        

                # results, logprobs = dsprover.prove(conjectures)
                # for i, result, logprobs in enumerate(zip(results, logprobs)):
                #     student_results.append({"problem": conjectures[i], "translated": lean_conjectures[i], "proof": result, "logprob": logprob})
                
                # for i in range(0, len(conjectures), cfg.get("batch_size")):
                #     batch = conjectures[i:min(i+cfg.get("batch_size"), len(conjectures))]
                #     conjecture_lean = convert_arith(conjecture, index, flag_matters=False)
                #     if conjecture_lean:
                #         result, logprob = dsprover.prove([conjecture_lean])[0]
                #         if result:
