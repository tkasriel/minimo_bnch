...
def prove (conjectures : list[str], debug: bool = False) -> list[tuple[str | None, float]]:
    ...
    if debug:
        print(f"tokenization took {time.time() - start_time}s")
        start_time = time.time()
    ...
    generated_tokens = outputs.sequences[:,input_lengths:]
    if debug:
        print (f"Inference/decoding took {time.time()-start_time}s")
        start_time = time.time()

    results = []
    for generated_i, transition_i in zip(generated_tokens, transition_scores):
...
if __name__ == "__main__":
    ...
    with distributed_state.split_between_processes(batches) as batch:
